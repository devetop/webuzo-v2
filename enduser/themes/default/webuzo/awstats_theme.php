...
function awstats_theme(){
	...
	global $theme, $globals, $l, $error, $errorlog, $domains_list, $error_log, $softpanel ,$done, $domain, $awstat_key;
			...
			$(".awstats_view").click(function() {
				...
				$.ajax({
					...
					success: function(data){
						...
						var awstat_key = data.split("_");  //Get the key
						data = awstat_key[0];
						...
						if(result == "1"){
							...
							window.open(wprotocol+"//"+ip+"/cgi-bin/awstats.pl?config="+msg+"&auth_key="+encodeURIComponent(awstat_key[1]));
						}
						if(result == "0"){
