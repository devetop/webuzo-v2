...
global $theme, $user, $logged_in, $globals, $l, $dmenus, $onload, $newslinks, $feeds, $softpanel, $iscripts, $catwise, $allcatwise, $soft, $classes_categories, $scripts, $apps, $apps_catwise, $user_sitepad, $ins_apps, $file_data, $act;
	...
	if(empty($globals['off_panel_link'])){
		$navbar_top['goto_control_panel']['fullscreen'] = '<td align="center" width="32" class="someclass" title="'.$l['go_cpanel'].'"><a href="'.$softpanel->theme['panel_url'].'">'.((!empty($softpanel->goto_cp_logo) && empty($user['user_defined_color']) && empty($globals['default_hf_text'])) ? '<img src="'.$theme['images'].$softpanel->goto_cp_logo.'" height="25" width="25" >' : '<i class="fa sai-control_panel fa-2x"></i>').'</a></td>';
		
		$navbar_top['goto_control_panel']['responsive'] = '<li><a href="'.$softpanel->theme['panel_url'].'">'.((!empty($softpanel->goto_cp_logo) && empty($user['user_defined_color']) && empty($globals['default_hf_text'])) ? '<img src="'.$theme['images'].$softpanel->goto_cp_logo.'" height="25" width="25" >' : '<i class="fa sai-control_panel fa-2x"></i>').'&nbsp;&nbsp;'.$l['go_cpanel'].'</a></li><br />';
	}
	...
	if(can_wordpress_manager()){
		$navbar_top['goto_wp_manager']['fullscreen'] = '<td align="center" width="32" class="someclass" title="'.$l['go_wp_manager'].'"><a href="'.$globals['ind'].'act=wordpress"><i class="fa sai-wp" style="font-size:1.8em;"></i></a></td>';
		
		$navbar_top['goto_wp_manager']['responsive'] = '<li><a href="'.$globals['ind'].'act=wordpress"><i class="fa sai-wp" style="font-size:1.8em;"></i>&nbsp;&nbsp;'.$l['go_wp_manager'].'</a></li><br />';
	}
	...
	if(!webuzo()){
		...
		if(empty($globals['off_sync_link']) && !aefer() && !ent()){
			$navbar_top['goto_sync']['fullscreen'] = '<td align="center" width="32" class="someclass" title="'.$l['go_sync'].'"><a href="'.$globals['ind'].'act=sync"><i class="fa sai-download fa-1x"></i></a></td>';
			
			$navbar_top['goto_sync']['responsive'] = '<li><a href="'.$globals['ind'].'act=sync"><i class="fa sai-download fa-1x"></i>&nbsp;&nbsp;'.$l['go_sync'].'</a></li><br />';
		}
		
		// Show the plans page if the original license is free
		if(!empty($globals['orig_is_free'])){
			$navbar_top['goto_plans']['fullscreen'] = '<td align="center" width="32" class="someclass" title="'.$l['go_plans'].'"><a href="'.$globals['ind'].'act=plans"><i class="fa sai-cart fa-1x"></i></a></td>';
			
			$navbar_top['goto_plans']['responsive'] = '<li><a href="'.$globals['ind'].'act=plans"><i class="fa sai-cart fa-1x"></i>&nbsp;&nbsp;'.$l['go_plans'].'</a></li><br />';
		}
...
if($ind != 'classes'){
	...
	function show_left_panel(combine){
		...
		if(can_show_sitepad()){
			echo 'str_html += \'<table border="0" cellpadding="0" cellspacing="0" id="softmain" width="100%"><tr id="head_sitepad" class="soft_cathead_slide"><td width="80%" class="soft_cathead" height="32" valign="middle"><a href="'.$globals['indmode'].'act=sitepad_overview"><i class="fa sai-pfx_icon fa-lg"></i>&nbsp;&nbsp;SitePad Website Builder</a></td></tr></table>\';';
		}
