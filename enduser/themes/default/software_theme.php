	...
	if(!empty($settings)){
		...
		foreach($settings as $group => $sets){
			...
			<div class="script_blocks">
				...
				$is_plugin_theme = array_keys(array_filter($all_sets));
				if($group == $l['ss']['select_plugins'] && !empty($all_sets[$is_plugin_theme[0]])){	
					$info['install'] .= '
					<div class="row">
						<div class="col-sm-5">
						...
						<div class="col-sm-7">
							...
							foreach($all_sets as $sk=> $av){
								...
								$info['install'] .= '
								<div class="col-sm-5">
									<input type="checkbox" name="sets_name[]" id="'.$sk.'" '.POSTchecked_Array('sets_name', $sk, (!empty($av['default_value']) ? true : false)).' value="'.$sk.'" />&nbsp;&nbsp;<span class="sai_head">'.str_replace('_admin', '', $sk).'</span>
			...
			<div id="advoptions" style="display:none;"><hr>';
				...
				if(empty($globals['off_upgrade_plugins']) && !empty($software['update_plugins'])){
					...
					<div class="row">
						...
						<div class="col-sm-7">
							<input type="checkbox" name="auto_upgrade_plugins" id="auto_upgrade_plugins" '.POSTchecked('auto_upgrade_plugins', (!empty($globals['force_upgrade_plugins']) ? $globals['force_upgrade_plugins'] : $globals['preselect_autoupgrade_plugins'])).(!empty($globals['force_upgrade_plugins']) ? 'disabled' : '').'/>
						</div>	
					</div><br />';
				}
				...
				if(empty($globals['off_upgrade_themes']) && !empty($software['update_themes'])){
					...
					<div class="row">
						...
						<div class="col-sm-7">
							<input type="checkbox" name="auto_upgrade_themes" id="auto_upgrade_themes" '.POSTchecked('auto_upgrade_themes', (!empty($globals['force_upgrade_themes']) ? $globals['force_upgrade_themes'] : $globals['preselect_autoupgrade_themes'])).(!empty($globals['force_upgrade_themes']) ? 'disabled' : '').'/>
						</div>	
					</div><br />';
				}
...
echo '<div class="bg">
	...
	<div class="row" style="margin-left:5px; margin-top:10px;">
		<div class="col-sm-1 col-xs-2 logo_style" style="padding:15px;">
			'.(!empty($custom_48) ? '<img src="'.$custom_48.'" alt="">' : '<img src="'.$globals['softimages'].'top15/48/'.$scripts[$soft]['softname'].'.png" >').'
		</div>
		<div class="col-sm-11 col-xs-10 details_style" style="padding-left:0px;">
			<div class="row">
				<div class="col-sm-12">
					<span class="sai_process_heading">'.$software['name'].'</span>'.($soft > 10000 ? '' : '&nbsp;&nbsp;&nbsp;<span class="someclass" title="'.$scripts[$soft]['ratings'].'">'.implode('', $ratings).'</span>').'
				</div>
			</div>
			...
			<div class="row" style="padding-top:2px;">
				<div class="col-sm-12" style="padding-left:20px;">
					'.(!empty($info['release_date']) ? $l['release_date'].' : <span><b>'.$info['release_date'].'</b></span>&nbsp;&nbsp;' : '').'
	...
	<div class="old_tab">
		<table width="100%" cellpadding="0" cellspacing="0" border="0" class="sai_tabs">
			<tr>
				<td><a href="javascript:tabs.tab(\'install\')" id="install" class="sai_insbut" style="border-right:0px;">'.$l['install'].'</a></td>
...
<div class="bg2">
	<div class="row">
		...
		<div class="col-sm-6 col-md-6 col-xs-12" style="padding-bottom:20px;">
			<div id="overview_img" style="padding:10px;"> <img src="'.$screenshot_src.'" class="img-responsive" alt=""> </div>	
		</div>
		<div class="col-sm-6 col-md-6 col-xs-12">': '<div class="col-sm-12 col-xs-12">').'
			...
			<div class="row">
				...
				<div class="col-sm-12 col-md-6 col-xs-12" style="margin-top:10px;">
					<div class="col-sm-2 col-md-2 col-xs-2">
						<a href="'.$info['support'].'"><font color="#447edf"><i class="fa sai-question fa-3x"></i></font></a>
					</div>
					<div class="col-sm-10 col-md-10 col-xs-10">
						<span class="sai_head">'.$l['software_support'].'</span><br />
						<a href="'.$info['support'].'" target="_blank">'.$l['support_link'].'</a>
					</div>
	...
	$info['overview'] .= '
		<table border="0" cellpadding="5" cellspacing="0" width="100%" class="table table-hover">
			<thead style="background:#EFEFEF;" class="sai_head2">
				<tr>
					<th><b>'.$l['link'].'</b></th>
					<th><b>'.$l['admin'].'</b></th>
					<th><b>'.$l['ins_time'].'</b></th>
					<th><b>'.$l['version'].'</b></th>
					<th style="text-align:center;"><b>'.$l['options'].'</b></th>
		...
		foreach($software['branch'] as $csid => $cver){
			...
			foreach($installations[$csid] as $ik => $iv){
				...
				<td><a href="'.(empty($globals['disable_sign_on']) && !empty($sign_on) ? $globals['ind'].'act=sign_on&insid='.$ik.'&autoid='.srandstr(32) : $iv['softurl'].'/'.(!empty($iv['admin_folder']) ? $iv['admin_folder'] : admin_folder($iv['sid']))).'" target="_blank"><img src="'.$theme['images'].'/admin.gif" /></a></td>
				<td>'.datify($iv['itime']).'</td>
				<td>'.$iv['ver'].(!empty($is_upgradable) ? ' <a href="'.$globals['ind'].'act=upgrade&insid='.$ik.'"><img src="'.$theme['images'].'updates.gif" alt="'.$l['upd_to'].' '.implode(' or ', $is_upgradable).'" title="'.$l['upd_to'].' '.implode(' or ', $is_upgradable).'" class="someclass"/></a>' : '').
				...
				<td align="right">
				'.(can_wordpress_manager($iv['sid']) ? '<a href="'.$globals['ind'].'act=wordpress&insid='.$ik.'" title="'.$l['wordpress_manager'].'" ><img src="'.$theme['images'].'wp-gray.png" height="23" width="23"/></a>&nbsp;' : '').'
				'.(!empty($can_publish) && ampps() ? '<a href="'.$globals['ind'].'act=ampps_publish&insid='.$ik.'" title="'.$l['ampps_publish'].'" ><img src="'.$theme['a_images'].'ampps_publish.png" height="23" width="23"/></a>&nbsp;' : '' ).'
				
				&nbsp'.(!empty($can_staging) ? (!empty($iv['is_staging']) ? '<a title="'.$l['pushtolive'].'" href="'.$globals['ind'].'act=pushtolive&insid='.$ik.'" class="someclass"><img src="'.$theme['images'].'pushtolive.png"/></a>&nbsp;' : '').
...
if(count($allcatwise[$software['type']][$software['cat']]) > 2 && $software['cat'] != 'others' && empty($globals['disable_related_scripts'])){
	...
	<div class="bg2">
		...
		<div class="row related_scripts" style="padding-top:10px; padding-left:10px;">';
