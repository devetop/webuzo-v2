...
foreach($installations as $sid => $installs){
	...
	foreach($installs as $ik => $iv){
		...
		'.(!empty($can_publish) && ampps() ? '<a href="'.$globals['ind'].'act=ampps_publish&insid='.$ik.'" title="'.$l['ampps_publish'].'" onclick="'.(ampps() && !is_ampps_premium() ? "ampps_premium_handle();return false" : '').'"><img src="'.$theme['a_images'].'ampps_publish.png" height="23" width="23"/></a>&nbsp;' : '' ).'&nbsp'
		...
		<td width="50"><input type="checkbox" class="shift_select_ins check_all_ins '.$iscripts[$sid]['softname'].'_check_ins" name="insids[]" id="'.$iv['softurl'].'" value="'.$ik.'">
		</td>
	</tr>';
