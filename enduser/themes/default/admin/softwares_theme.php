...
if(!empty($globals['lictype'])){
	echo '<td width="40%"><input type="submit" name="updatesoft" value="'.$l['updatesoft'].'" class="flat-butt" id="updatesoft_but" disabled="true" /></td>';
}
	...
	foreach($catscripts as $cat => $cv){
		...
		foreach($cv as $k => $v){
			...
			echo '<tr>
				...
				<td width="5%" class="sai_bboxtxt" align="center"><input type="checkbox" class="shift_select_installed" id="soft_'.$k.'" name="soft_'.$k.'" '.POSTchecked('soft_'.$k, !empty($iscripts[$k])).' onmouseover="showtip(\''.$l['soft_enable'].' '.$scripts[$k]['name'].'\', this)" /></td>
				'.(empty($globals['pre_download_all']) ? '<td width="5%" class="sai_bboxtxt" align="center"><input type="checkbox" class="shift_select_pre_download" id="soft_pre_'.$k.'" name="soft_pre_'.$k.'" '.POSTchecked('soft_pre_'.$k, !empty($iscripts[$k]['pre_down'])).' onmouseover="showtip(\''.$l['pre_down'].' '.$scripts[$k]['name'].'\', this)" /></td>' : '').'
